...
print(">>> Measuring PSO Risk of Non-Privatized Data")

## Note: jitter_factor_array is set in the pso_utils file
## Also, since the non-private approach uses the data as is with no randomness, we simply run the following

nonprivate_risks = pd.DataFrame(pso_nonpriv_run(features_l2))
...
print(">>> Measuring PSO Risk of Privatized Data")

private_repo = []

for B in B_params:
	print("----------------------------------------------")
	print("*-> Running  B = " + str(B))
	priv_res = sim_full_run(features_l2, sim_size, epsilon_1, delta_1, epsilon_2, delta_2, k, B)
...
nonprivate_risks.to_csv('nonprivate_pso_metrics.csv', index = False)

privacy_param_string = "ep1_" + str(epsilon_1) + "_del1_" + str(delta_1) + "_ep2_" + str(epsilon_2) + "_del2_" + str(delta_2) + "_k_" + str(k)
private_risks.to_csv('private_pso_metrics_' + privacy_param_string + '.csv', index = False)
